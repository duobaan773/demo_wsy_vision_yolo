...
std::string makeImageResultPath(
...
std::string makeVideoResultPath(
    const std::string& input_path)
{
    const std::filesystem::path source_path(
        input_path
...
int main(int argc, char* argv[])
{
    ...
    std::cout
        << " RoboMaster 2027 Stage2 Detect System\n";
    ...
    {
        std::cerr
            << "[Main] Failed to initialize detector.\n";
        ...
        if (input_mode == "video")
{
    if (!video_writer_initialized)
    {
        const std::string result_path =
            makeVideoResultPath(input_path);
        ...
        {
            std::cerr
                << "[Main] Failed to open result video: "
                << result_path << '\n';
        ...
        std::cout
            << "[Main] Result video will be saved to: "
            << result_path << '\n';
    }
    ...
    video_writer.write(frame);
        ...
        {
            const std::string result_path =
                makeImageResultPath(input_path);
